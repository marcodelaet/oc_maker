...
export function buildPdfDefinition(campaign, options, fees, logoDataUri = null) {
  ...
  const logo = logoDataUri || logoBase64Sync();
  ...
  return {
    ...
    content: [
      ...buildHeaderBlock(documentTitle, documentId, logo),
      {
        table: { widths: ["*", "*"], body: metaBody },
        layout: "noBorders",
        margin: [0, 0, 0, 10],
      },
      {
        ...
        layout: {
          ...
          hLineColor: () => "#dddddd",
          vLineColor: () => "#dddddd",
          fillColor: (row) => (row === 0 ? PDF_COLORS.tableHeader : null),
        },
      },
...
export async function downloadPdf(campaign, options, fees) {
  const { loadLogoBase64 } = await import("./pdfLayout.js");
  ...
  return new Promise((resolve, reject) => {
    pdfMake.createPdf(docDef).download(filename, () => resolve(filename), reject);
  });
}
